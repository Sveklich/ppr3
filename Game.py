...
def loadsave():
    ...
    print("Вы ", {player1.name}, '?(Да/Нет)')
    sv = input()
    if sv == "Да":
        print("Что делать с сохранение?")
        print("Удалить")
        print("Продолжить")
        if input() == "Удалить":
            setup_game()
            ...
            player1.name = data["name"]
            player1.astrological = data["astro"]
            room_solved = data["solv"]
            player1.solves = data["solves"]
            main_game_loop()
    else:
        setup_game()
def clear_console():
...
def title_screen_options():
    ...
    if option.lower() == "играть":
        ...
        help_menu()
    elif option.lower() == "сохранения":
        loadsave()
    while option.lower() not in ['играть', 'помощь', 'выход', 'сохранения']:
...
def prompt():
    ...
    if action.lower() == "выйти":
        ...
        new_data = {
            "name": player1.name,
            "astro": player1.astrological,
            "solv": room_solved,
            "solves": player1.solves
        }
        existing_data.update(new_data)
        with open('saves.json', 'w') as file:
            json.dump(existing_data, file, indent=4)
        datac = [
            [player1.name, player1.astrological, player1.solves]
        ]
...
def setup_game():
    ...
    question1 = "Здравствуйте, как ваше имя(на латинице)?\n"
